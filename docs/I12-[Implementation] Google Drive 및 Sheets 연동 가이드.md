...
### 8.1. n8n 워크플로우 테스트 (✅ 완료 2026-01-13)

- [x] WF01: 발명 제안서 제출 → Sheets 행 생성 확인
- [x] WF01: 발명 제안서 제출 → Drive 01_발명제안서 파일 생성 확인
- [x] WF03: 명세서 생성 → Drive 03_명세서초안 파일 생성 확인
- [x] WF03: 명세서 생성 → Sheets 상태 "reviewing" 업데이트 확인
- [x] WF04: 검수 승인 → Drive 04_승인문서 파일 이동 확인
- [x] WF04: 검수 승인 → Sheets 상태 "approved" 업데이트 확인
- [x] WF04: 검수 반려 → Drive 05_반려문서 파일 이동 확인

### 8.2. 프론트엔드 테스트 (✅ 부분 완료)

- [x] 대시보드: localStorage 데이터 로드 확인
- [x] 대시보드: 통계 정확성 확인 (승인 완료 카운터 정상)
- [x] 검수 페이지: 상태 변경 후 localStorage 반영 확인 (버그 수정 완료)
- [ ] 대시보드: Sheets API 직접 연동 (WF06 구현 필요)
- [ ] 오프라인: localStorage 캐시 동기화 (향후 구현)
